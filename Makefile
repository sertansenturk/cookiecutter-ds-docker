...
build: 
	DOCKER_BUILDKIT=${BUILDKIT} POSTGRES_UID=${POSTGRES_UID} POSTGRES_GID=${POSTGRES_GID} docker-compose build ${BUILD_OPTS}
...
up: 
	...
	JUPYTER_UID=${JUPYTER_UID} POSTGRES_UID=${POSTGRES_UID} POSTGRES_GID=${POSTGRES_GID} docker-compose up ${UP_OPTS}
run:
	mkdir -p ${MLFLOW_ARTIFACT_STORE} ${POSTGRES_STORE}
	POSTGRES_UID=${POSTGRES_UID} POSTGRES_GID=${POSTGRES_GID} docker-compose run ${RUN_OPTS}
